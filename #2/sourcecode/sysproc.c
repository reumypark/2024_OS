...
sys_set_proc_info(void)
{
  ...
  //Fetch the arguments
  if (argint(0, &q_level) < 0 || argint(1, &cpu_burst) < 0 || argint(2, &cpu_wait_time) < 0 ||
      argint(3, &io_wait_time) < 0 || argint(4, &end_time) < 0) {
      return -1;
  }
  ...
  if (!curproc) {
    cprintf("Error: myproc() returned null!\n");
    return -1;
  }
  
  curproc->q_level = q_level;
  curproc->cpu_burst = cpu_burst;
  curproc->cpu_wait = cpu_wait_time;
  curproc->io_wait_time = io_wait_time;
  curproc->end_time = end_time;
